...
def test_jdqz_largest_magnitude_lowdim(dtype):
    ...
    alpha, beta = jdqz.jdqz(a, b, k, Target.LargestMagnitude, tol=tol, subspace_dimensions=[10, 15])
    jdqz_eigs = numpy.array(sorted(alpha / beta, key=lambda x: -abs(x)))
...
def test_Epetra_lowdim():
    ...
    alpha, beta = jdqz.jdqz(a2, b2, k, Target.LargestMagnitude, tol=tol, subspace_dimensions=[10, 15], interface=interface)
    jdqz_eigs = numpy.array(sorted(alpha / beta, key=lambda x: -abs(x)))
